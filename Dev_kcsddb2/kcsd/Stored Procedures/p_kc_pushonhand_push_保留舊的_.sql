...
-- ==========================================================================================
CREATE      PROCEDURE [kcsd].[p_kc_pushonhand_push(保留舊的)] @pm_strt_date datetime=NULL, @pm_stop_date datetime=NULL,
...
*/
	...
	BEGIN
		...
		IF	(@wk_over_amt > 0
		OR	@wk_pay_sum > 0 ) and 
		left(@wk_pusher_code,1) ='P' 
			INSERT	#tmp_pushonhand
				(kc_case_no, kc_pusher_code, kc_strt_date, kc_close_date,
				kc_over_amt, kc_pay_sum, kc_intr_sum, kc_break_sum, kc_delay_code,kc_pay_sum1)
			VALUES	(@wk_case_no, @wk_pusher_code, @wk_strt_date, @wk_close_date,
				@wk_over_amt, @wk_pay_sum, @wk_intr_sum, @wk_break_sum, @wk_expt_cont,@wk_pay_sum1)
		ELSE	-- 刪除 0
			DELETE
			FROM	#tmp_pushonhand
			WHERE	kc_case_no = @wk_case_no
