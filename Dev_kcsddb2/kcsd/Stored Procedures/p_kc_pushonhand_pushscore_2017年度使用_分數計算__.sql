-- ==========================================================================================
-- 20170424 調整M1回收標準
-- 20170407 修改報表MP的違約金也要算獎金/修正獎金發放上限為25%
-- 20170315 增加獎金計算
...
CREATE PROCEDURE [kcsd].[p_kc_pushonhand_pushscore(2017年度使用[分數計算]])] @pm_strt_date datetime=NULL, @pm_stop_date datetime=NULL	,@pm_area_code varchar(100)=NULL,@pm_run_code varchar(10)=NULL,@pm_group_code varchar(10)=NULL
AS
...
--先GROUPBY起來
IF	@pm_group_code = 'NOAREA'
BEGIN
	Insert into #tmp_pushonhand3
	SELECT  
	kc_pusher_code,
	'',
	'',
...
--計算個人得分並存入Temp3
DECLARE @count int
SELECT @count = count(*) from #tmp_pushonhand4
IF @count > 0
BEGIN
	--4
	DECLARE	cursor_pushonhand	CURSOR
	...
	BEGIN
		...
		UPDATE #tmp_pushonhand4 SET kc_amount_point = @wk_m0_amount_point ,kc_recover_point = @wk_m0_recover_point WHERE kc_pusher_code = @wk_pusher_code AND kc_delay_code = '0'
		UPDATE #tmp_pushonhand4 SET kc_amount_point = @wk_m1_amount_point ,kc_recover_point = @wk_m1_recover_point WHERE kc_pusher_code = @wk_pusher_code AND kc_delay_code = '1'
		UPDATE #tmp_pushonhand4 SET kc_amount_point = @wk_m2_amount_point ,kc_recover_point = @wk_m2_recover_point WHERE kc_pusher_code = @wk_pusher_code AND kc_delay_code = '2'
		UPDATE #tmp_pushonhand4 SET kc_amount_point = @wk_m3_amount_point ,kc_recover_point = @wk_m3_recover_point WHERE kc_pusher_code = @wk_pusher_code AND kc_delay_code = '3'
		UPDATE #tmp_pushonhand4 SET kc_amount_point = 0 ,kc_recover_point = 0 WHERE kc_pusher_code = @wk_pusher_code AND kc_delay_code = 'P'

		--計算得分
		SELECT @wk_amount_point  = sum(kc_amount_point) * 0.15 from #tmp_pushonhand4 WHERE kc_pusher_code = @wk_pusher_code AND kc_delay_code <> 'P'
		SELECT @wk_recover_point = sum(kc_recover_point)* 0.1  from #tmp_pushonhand4 WHERE kc_pusher_code = @wk_pusher_code AND kc_delay_code <> 'P'
		SELECT @wk_scores_point = @wk_amount_point  + @wk_recover_point + (@wk_count -500)/50
		SELECT @wk_scores_point1 = @wk_amount_point  + @wk_recover_point
...
DECLARE	cursor_pushonhand	CURSOR
FOR	/*
	SELECT kc_pusher_code,kc_area_code,CASE WHEN sum(distinct kc_scores_point2)>=60 THEN ROUND(((12 + FLOOR(sum(distinct kc_scores_point2) - 60))*0.01) * sum(kc_break_sum),0) ELSE 0 END as kc_bonus_amt
	from #tmp_pushonhand3
	where kc_delay_code <> 'P'
	group by kc_pusher_code,kc_area_code
	order by kc_area_code,kc_pusher_code
	*/
	SELECT kc_pusher_code,kc_area_code,
	CASE WHEN sum(distinct kc_scores_point2)>=60 THEN CASE WHEN ((12 + FLOOR(sum(distinct kc_scores_point2) - 60))*0.01)>0.25 THEN sum(kc_break_sum) * 0.25 ELSE ROUND(((12 + FLOOR(sum(distinct kc_scores_point2) - 60))*0.01) * sum(kc_break_sum),0) END ELSE 0 END as kc_bonus_amt
	from #tmp_pushonhand3
	--where kc_delay_code <> 'P'
	group by kc_pusher_code,kc_area_code
	order by kc_area_code,kc_pusher_code
OPEN cursor_pushonhand
FETCH NEXT FROM cursor_pushonhand INTO @wk_pusher_code,@wk_area_code,@wk_bonus_amt
SET @i = 1
WHILE (@@FETCH_STATUS = 0)
BEGIN
	UPDATE #tmp_pushonhand3 SET kc_bonus_amt = @wk_bonus_amt WHERE kc_pusher_code = @wk_pusher_code and kc_area_code = @wk_area_code
