CREATE PROCEDURE [kcsd].[p_kc_cr_old_loanpayment2(停用)]
AS
...
BEGIN
	SELECT	@wk_case_no
	SELECT	@wk_loan_perd = kc_loan_perd, @wk_strt_date = kc_strt_date,
		@wk_perd_fee = kc_perd_fee
	FROM	kcsd.kc_customerloan2
	...
	BEGIN
		INSERT	kcsd.kc_loanpayment2
			(kc_case_no, kc_perd_no, kc_expt_date, kc_expt_fee)
		VALUES	(@wk_case_no, @wk_counter+1,
			DATEADD(month, @wk_counter, @wk_strt_date), @wk_perd_fee)
