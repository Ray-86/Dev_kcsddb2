...
-- =============================================
CREATE PROCEDURE [kcsd].[p_kc_pushassign_M2M1toM1M0] @pm_run_code varchar(10)=NULL, @pm_pusher_date datetime=NULL
AS
...
SELECT kc_case_no,kc_pusher_type FROM (
	...
	SELECT	c.kc_case_no,c.kc_over_amt,'P' as kc_pusher_type
		...
		AND  EXISTS
			...
			and kc_delay_code in ('M3'))
		AND datediff(day,(SELECT DATEADD(day, -90, MAX(kc_pay_date)) as kc_pay_date FROM kcsd.kc_loanpayment WHERE kc_pay_type = '7' AND kc_pay_date < GETDATE()),kc_dday_date) >= 0
	union
	...
	SELECT	c.kc_case_no,c.kc_over_amt,'P' as kc_pusher_type
		...
		AND  EXISTS
			...
			WHERE	p.kc_case_no = c.kc_case_no
			AND	p.kc_stop_date IS  NULL
			and kc_delay_code in ('M2'))
		AND datediff(day,(SELECT DATEADD(day, -60, MAX(kc_pay_date)) as kc_pay_date FROM kcsd.kc_loanpayment WHERE kc_pay_type = '7' AND kc_pay_date < GETDATE()),kc_dday_date) >= 0
	union
