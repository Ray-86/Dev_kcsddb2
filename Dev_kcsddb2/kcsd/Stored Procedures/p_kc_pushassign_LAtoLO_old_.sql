-- ==========================================================================================
-- 2017-05-31 去除排除L04 條件
-- 2016-11-28 新增L08,09 舊件改為每人 隨機TOP150
-- 2016-02-03 LA轉L(排除L04)舊件 每人 隨機TOP200
-- ==========================================================================================

CREATE                PROCEDURE [kcsd].[p_kc_pushassign_LAtoLO(old)]
...
-- 預設為LA
SELECT	@wk_case_no=NULL, @wk_circle_count = 0, @wk_lawyer_code = NULL,@wk_pusher_count = 1
-- 今天
...
--取得LA 開始時間排序 其餘 亂數 排序
DECLARE	cursor_case_no_LAtoLO	CURSOR
FOR	SELECT TOP 1350 p.kc_case_no
	FROM kcsd.kc_pushassign p
	WHERE p.kc_pusher_code = 'LA'
	AND p.kc_stop_date IS NULL
   	ORDER BY p.kc_strt_date,NEWID()

print @wk_pstrt_date
OPEN cursor_case_no_LAtoLO
FETCH NEXT FROM cursor_case_no_LAtoLO INTO @wk_case_no
...
BEGIN
	...
	--派給L01~L09
	SET @wk_lawyer_code = 'L0' + CONVERT(varchar(1),@wk_pusher_count)
