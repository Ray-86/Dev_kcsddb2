...
-- =============================================
CREATE PROCEDURE  [kcsd].[p_kc_updatemblist] @wk_cp_no varchar(10),@wk_type varchar(2)
...
select @wk_cp_no = kc_cp_no from  [kcsd].[kc_customerloan] where kc_case_no = @wk_cp_no

END

--select * from  kcsd.kc_chkmblist 


DELETE FROM kcsd.kc_chkmblist  WHERE kc_cp_no = @wk_cp_no and Substring(kc_mobile_type,1,2) = @wk_type

insert kcsd.kc_Chkmblist
select convert(varchar, getdate(), 112) as CreateDate,* from
(
--------C包含黑名單
--------B轉直
select  kc_cp_no, kc_mobile_type , kc_mobile_no
from 
(
--------A目標
...
(kc_cp_date >= dateadd(YYYY,-2,getdate()) or isNULL(kc_loan_stat,'') <> 'C')
...
--------B轉直
UNION
select '' as kc_cp_no,'Black' as kc_mobile_type,kc_mobile_no from [kcsd].[kc_mobileblacklist]
--------C包含黑名單
) as target
where LEN(kc_mobile_no) > 6 and kc_cp_no = @wk_cp_no and Substring(kc_mobile_type,1,2) = @wk_type
