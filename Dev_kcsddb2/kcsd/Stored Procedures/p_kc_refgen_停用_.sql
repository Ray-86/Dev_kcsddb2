-- ==========================================================================================
-- 08/12/06 KC: add 2 more column at dstdb.kc_customerloan to match new changes
-- KC: last pay date 5/31/06
-- FOR DY2
-- ==========================================================================================
CREATE      PROCEDURE [kcsd].[p_kc_refgen(停用)] 
...
FOR	SELECT	kc_case_no
	...
	WHERE	(	kc_loan_stat = 'C'
		OR	kc_loan_stat = 'E')
--	AND	kc_case_no LIKE '93%'			-- 已執行 8/12/06
--	AND	kc_case_no LIKE '92%'			-- 已執行
--	AND	kc_case_no BETWEEN '9300001' AND '9300010'

--	AND	kc_case_no LIKE '92%'			-- 已執行
--	AND	kc_case_no LIKE '93%'			-- 已執行
	AND	kc_case_no LIKE '94%'			-- 已執行
...
-- reference
		...
		WHERE	kc_case_no = @wk_case_no
-- customerloan
		INSERT	refdb.kcsd.kc_customerloan
		SELECT	*
		FROM	kcsd.kc_customerloan
		WHERE	kc_case_no = @wk_case_no
-- loanpayment	
		INSERT	refdb.kcsd.kc_loanpayment
		SELECT	*
		FROM	kcsd.kc_loanpayment
		WHERE	kc_case_no = @wk_case_no
-- push
		INSERT	refdb.kcsd.kc_push
		SELECT	*
		FROM	kcsd.kc_push
		WHERE	kc_case_no = @wk_case_no
	END
