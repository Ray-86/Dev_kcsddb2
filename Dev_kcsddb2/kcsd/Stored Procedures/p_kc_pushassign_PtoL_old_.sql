...
CREATE                PROCEDURE [kcsd].[p_kc_pushassign_PtoL(old)]
...
DECLARE	cursor_case_no_PtoL	CURSOR
FOR	
...
OPEN cursor_case_no_PtoL
FETCH NEXT FROM cursor_case_no_PtoL INTO @wk_case_no
...
--檢查最後估車入帳日
	--SELECT @wk_fig0_date = MAX(kc_pay_date) FROM kcsd.kc_loanpayment WHERE kc_case_no = @wk_case_no AND kc_rece_code = 'A2'
	--SELECT @wk_fig0_amt = Count(kc_case_no) FROM kcsd.kc_appraised WHERE kc_case_no = @wk_case_no AND kc_confirm_fig = 'P'
--檢查是否有選(轉協商部)
--	SELECT @wk_fig3_amt = Count(kc_case_no) from kcsd.kc_lawstatus where kc_case_no = @wk_case_no and kc_law_code = 'V' and kc_law_date between DATEADD(mm, DATEDIFF(mm,0,getdate()), -7) and DATEADD(mm, DATEDIFF(mm,0,getdate()), 3)
--檢查估車日後是否有查訪完成
--	SELECT @wk_fig4_amt = Count(kc_case_no) from kcsd.kc_apptschedule where kc_case_no = @wk_case_no and kc_appt_type in( 'B','D') and kc_visit_outc is not null and kc_appt_date >= (select max(kc_law_date) from kcsd.kc_lawstatus where kc_case_no =  @wk_case_no and kc_law_code ='M' AND kc_law_amt IS NOT NULL)

	IF	(
		--((@wk_fig0_date IS NULL AND @wk_over_amt > 0 AND DATEDIFF(month, @wk_dday_date, @pm_age_date) >= 4)	--非估車件 帳齡4個月(含)以上
		--OR (@wk_fig0_date IS NOT NULL AND @wk_fig0_amt > 0 AND DATEDIFF(month, @wk_fig0_date, @pm_age_date) >= 2 )) --或估車件 最後估車入帳日在2個月(含)以上
		DATEDIFF(month, @wk_dday_date, @pm_age_date) >= 4										--帳齡4個月(含)以上
		AND (@wk_lpay_date IS NULL OR DATEDIFF(month, @wk_lpay_date, @pm_age_date) >= 2) 		--最近繳款日至前一個月月底在2個月(含)以上
		AND	@wk_fig1_amt > 0	--沒寄信不派
		AND	@wk_fig2_amt > 0	--沒查訪不派
	...
	BEGIN
		...
		--END
		--1~9
		IF @wk_pusher_count > 9
		BEGIN
			SELECT @wk_pusher_count = 1
		END

		INSERT	#tmp_pushassign_PtoL
			(kc_case_no, kc_loan_stat,kc_expt_sum, kc_arec_amt,kc_dday_date,kc_lpay_date,kc_age_date,
		...
		VALUES	(@wk_case_no,@wk_loan_stat, @wk_over_amt, @wk_arec_amt,@wk_dday_date, @wk_lpay_date, @pm_age_date,
...
DEALLOCATE	cursor_case_no_PtoL
