/* 10/11/05 KC: 停止Monthly派件 */
CREATE                     PROCEDURE [kcsd].[p_kc_pushassign_monthly(保留舊的)]
...
BEGIN
	...
	BEGIN
		...
		INSERT	#tmp_assign
		VALUES	(@wk_case_no, @wk_pusher_code, @wk_pusher_code2)
	END
	FETCH NEXT FROM cursor_case_no INTO @wk_case_no
END
...
SELECT	t.*, e.DelegateName /*, a.kc_delay_code */
FROM	#tmp_assign t, kcsd.Delegate e /*, kcsd.kc_pushassign a */
WHERE	t.kc_pusher_code2 = e.DelegateCode
/*AND	t.kc_case_no = a.kc_case_no
AND	a.kc_stop_date IS NULL */
/*ORDER BY t.kc_pusher_code2, t.kc_case_no*/
