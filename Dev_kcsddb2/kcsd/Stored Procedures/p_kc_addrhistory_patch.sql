CREATE PROCEDURE [kcsd].[p_kc_addrhistory_patch]
AS
/* ª]ºÐñFªb inser trigger Ñ[ñJ insert address history,
   ▒N®Êª│ÁL address history ¬║case ©╔ÑRñJ address history */
...
FOR	SELECT	kc_case_no
	...
	WHERE	c.kc_loan_stat <> 'C'
	AND	NOT EXISTS (SELECT a.kc_id_no
				FROM	kcsd.kc_addresshistory a
				WHERE 	a.kc_id_no = c.kc_id_no )
